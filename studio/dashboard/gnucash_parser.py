...
UPCOMING_PATTERNS = [
    # (keyword_in_desc, label, day_of_month, type, approx_eur)
    ('Autonomos',  'Autónomos (Social Security)', 29, 'fixed', 381),
    ('Movistar',   'Movistar (Phone/Internet)',    1,  'fixed', 108),
    ('Crashplan',  'Crashplan Backup',             10, 'fixed', 43),
    ('Gsuite',     'Google Workspace',             3,  'fixed', 8),
    ('Cloudways',  'Cloudways Hosting',            7,  'fixed', 11),
    ('Asana',      'Asana',                        12, 'fixed', 27),
    ('Claude',     'Claude (Anthropic)',            23, 'fixed', 18),
    ('Github',     'GitHub Copilot',               15, 'fixed', 9),
    ('Algolia',    'Algolia',                      19, 'fixed', 4),
]
...
def parse(filepath, usd_rate=0.92, gbp_rate=1.17):
    ...
    recent_income = [
        {'date': r['date'], 'desc': r['desc'], 'eur': round(abs(r['eur']), 2), 'cur': r['cur'], 'amount': round(abs(r['val']), 2)}
...
def _compute_upcoming(rows, today, owner_draw):
    ...
    # Pattern-based recurring bills
    exp_rows = [r for r in rows if r['type'] == 'EXPENSE' and r['val'] > 0 and r['date'] >= '2025-01-01']

    for keyword, label, typical_day, bill_type, approx_eur in UPCOMING_PATTERNS:
        ...
        # Average monthly total (handles vendors with multiple transactions/month e.g. Movistar)
