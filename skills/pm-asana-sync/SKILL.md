---
name: pm-asana-sync
description: Sync the assignee's Asana tasks to a local offline mirror. For the current project, runs the studio sync script to update asana-mirror.md, then reads and summarises the result. Use at the start of any work session or when asked to sync.
...
This updates `asana-mirror.md` and `asana-ids.json` in the project root.

---

## Step 3 — Read the updated mirror

Read `asana-mirror.md`. Report:
...
- Local IDs (e.g. MCF-001) are assigned sequentially and stored in `asana-ids.json`.
